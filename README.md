...
- **自动更新** — 更新源写死官方仓库,开箱即用(tauri-plugin-updater)
...
> **管理员权限(首次弹一次,永久持久化)**:
> - **首次启动**自动弹一次 UAC,创建 Windows 计划任务(`登录时最高权限`);
> - 之后每次启动(含手动双击、开机登录)都通过计划任务**静默提权**,
>   手动/自动清理直接生效,**不再弹任何 UAC**;
> - 若首次拒绝了 UAC,不会反复弹;可在「设置 → 开机静默自启」手动开启。
